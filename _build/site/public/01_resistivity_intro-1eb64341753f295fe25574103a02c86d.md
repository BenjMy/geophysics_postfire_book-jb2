...
```{code-cell} python3
# Example: compute geometric factor K for a Wenner array (spacing a)
import numpy as np

def wenner_K(a):
    """Geometric factor for a Wenner array with electrode spacing a (m)."""
    return 2 * np.pi * a

spacings = np.array([0.25, 0.5, 1.0, 2.0, 5.0])  # metres
K = wenner_K(spacings)

for s, k in zip(spacings, K):
    print(f"a = {s:5.2f} m  →  K = {k:7.3f} m")
```
...
```{code-cell} python3
# Illustrative resipy workflow (requires the geophysics-course environment)
...
# k.invert()
# k.showResults()
print("resipy project object created.")
```
