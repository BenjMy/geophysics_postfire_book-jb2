...
Applied geophysics operates by detecting **physical contrasts** between materials — measuring spatial variations in intrinsic physical properties from the surface. Every rock, soil layer, or fluid possesses characteristic properties, including:
...
These contrasts serve **two fundamentally different but complementary purposes**:

### 1. Imaging structure

Some subsurface features change very slowly or not at all on human timescales — the depth to bedrock, the thickness of soil horizons, the geometry of geological layers, or the architecture of the root zone. Geophysics can **map this static skeleton** of the Critical Zone in a non-invasive way, providing spatial context that no amount of point sampling can replicate. A single well or soil core tells you what is there at one location; a geophysical survey tells you how it varies across the landscape.

```{admonition} The "Anomaly"
:class: tip
The physical contrast measured in geophysical surveys is often referred to as an **anomaly** — any location where a measured property deviates from the expected background value. Anomalies can vary in shape, size, and intensity, and may be structural (a buried rock layer) or dynamic (a wetting front after rain). Distinguishing between the two is one of the central interpretive challenges of applied geophysics.
```
...
Other subsurface processes are rapid, transient, and spatially localised — a rainfall infiltration front moving downward through the soil, water draining preferentially through a crack, roots extracting moisture from a specific depth during a hot afternoon. These **fluxes leave a physical signature** that geophysics can detect, particularly when measurements are repeated over time. This is where time-lapse strategies become essential: the change between two surveys reveals the flux that caused it.



A range of geophysical methods exists, each responding to one or more specific physical properties and offering unique insights into subsurface structure or dynamics. The [table below](#geophysical-methods-table) summarises the main methods organised by the physical property they measure. The measured contrasts are ultimately sensitive to the soil properties we care about:
...
(More detailed on [ER concept](er-concept#how-to-translate-er-to-another-proxy-of-interest) and [ERT physical principles fundation](resistivity-introduction))
...
The [figure below](#loiseau2023geophysical) shows the combination of geophysical techniques in the context of ecology (some methods are not explicity describe here, the reader is invited to refer to 	{cite}`loiseau2023geophysical` for more details : 
...
### A growing approach driven by technology

Time-lapse geophysics has historically been limited by the cost and logistical burden of deploying instruments repeatedly in the field. This is rapidly changing: sensors are becoming **smaller, cheaper, and increasingly autonomous**, enabling permanent electrode arrays and dataloggers to be left in place for months or years and triggered remotely. What once required a field crew and a day of work can now be scheduled as an overnight automated acquisition. As a result, time-lapse strategies are moving from occasional snapshots to **near-continuous subsurface monitoring**, generating datasets that more closely resemble the temporal resolution of meteorological or hydrological records.

```{admonition} Why does this matter?
:class: tip
Cheaper instruments and automated acquisition mean that time-lapse geophysics is no longer restricted to large research projects. It is increasingly accessible to environmental consultancies, restoration programmes, and long-term ecological observatories — broadening the range of sites and questions that can be addressed.
```

### Vegetation: a particularly dynamic target

This temporal resolution is especially important when the system under study includes **living vegetation**. Plants are not passive components of the Critical Zone — they actively redistribute water through:

- 🌿 **Transpiration**: roots extract soil water daily, creating diurnal cycles of moisture depletion that propagate downward through the profile
- 🍂 **Seasonal phenology**: the contrast between a leafed-out canopy in summer and a dormant one in winter drives dramatic shifts in evapotranspiration demand and therefore in soil moisture patterns
- 🌧️ **Interception and redistribution**: canopy structure controls how rainfall reaches the soil surface, producing spatial heterogeneity that evolves as vegetation recovers after disturbance
- 🌱 **Root growth**: as vegetation re-establishes after fire, expanding root networks progressively access deeper water stores, altering the depth and pattern of moisture anomalies

These processes unfold across **timescales ranging from hours to years**, and a single geophysical survey — however detailed — captures only one frozen moment. Time-lapse acquisition is therefore not just a technical refinement but a **conceptual necessity** when vegetation dynamics are part of the question being asked.

```{note}
In post-fire recovery monitoring, this is particularly relevant: the return of vegetation is one of the primary goals of restoration, yet it is also one of the main drivers of subsurface change. Time-lapse geophysics offers a way to track both simultaneously — linking what is visible above ground to what is happening in the soil and root zone below.
```
